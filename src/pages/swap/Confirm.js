...
import {getAmountOut, getAllowance} from '../../service/bscCore';
...
const Confirm = () => {
    const {startTokenSwap, targetTokenSwap, swapAmount, gasPrice, gasLimit}= React.useContext(SwapContext);
    const [expectedRetern, setExpectedReturn]= useState(0);
    ...
    React.useEffect(async ()=>{
        var expectedReterns= await getAmountOut(targetTokenSwap.addr, startTokenSwap.addr, swapAmount);
        var unitPrice= await getAmountOut(targetTokenSwap.addr, startTokenSwap.addr, 1);
        var gasFee= gasPrice*gasLimit/Math.pow(10, 9);
        var bnbPrice= await getAmountOut(BUSDAddr, WBNBAddr, 1);
        setGasFee(gasFee);
        setExpectedReturn(parseFloat(expectedReterns).toFixed(6));
        setUnitPrice(parseFloat(unitPrice).toFixed(6));
        setBnbPrice(parseFloat(bnbPrice).toFixed(6));
        if(startTokenSwap.name== "BNB"){
            setApproved(true);
        }else{
    ...
    return (
        <>
            <div style={{
                ...
                <Text style={{
                    marginBottom: "9px"
                }}>{swapAmount} {startTokenSwap.name}</Text>
                <SwapDownIcon width={12} height={15} style={{
                    marginBottom: "12px"
                }}/>
                <Text style={{
                    marginBottom: "12px"
                }}>{targetTokenSwap.name}</Text>
                <Text style={{
                    fontSize: "24px",
                    marginBottom: "49px",
                }}>{expectedRetern}</Text>
                <div style={{
                    ...
                    <Text style={{
                        marginRight: `11px`
                    }}>1 {startTokenSwap.name} = {unitPrice} {targetTokenSwap.name}</Text>
                    <RefreshIcon />
                </div>
                <BillingHighLight>
