...
const ComboBox = ({list = defaultList, onChange = () => {}, selected = 0, ...props}) => {
    ...
    return (
        <SelectedGroup className={`${open ? "open" : ""}`} onClick={() => setOpen(oldVal => !oldVal)} {...props}>
            <Selected>
            <img  width= "20px" src={list[selectedItem].img}/>
            <Text>{list[selectedItem].name}</Text>
                <Down width={"13px"} height={"8px"}/>
            </Selected>
            <ListGroup>
            {list.map((item, id) => (
                <Item key={id} onClick={() => setSelectedItem(id)}><img style= {{marginRight:'5px'}} width= "20px" src={item.img}/> {item.name}</Item>
            ))}
            </ListGroup>
        </SelectedGroup>
    )
